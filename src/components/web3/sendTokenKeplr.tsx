...
export function SendTokenKeplr({
  ...
  const { checkKeplr, sendToken, chains, isSelectedWallet, selectedKey } =
    useWeb3Context();
  const { user, refetch, refreshUserChains } = useUserData();
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChangeIntern}>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Send Token (Client Only)</DialogTitle>
            <DialogDescription>
